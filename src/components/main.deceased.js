...
function Main1(Props){
	...
	const[modifiedShare, setModifiedShare] = useState("")
	...
	async function modifyPayeeShares(){
		...
		if(!willNumber1){
			toast("No Will ID Specified")
		}
		if(!payeeAddress){
			toast("No Payee Address Specified")
		}
		if(!modifiedShare){
			toast("No Shares Specified")
		}
        ...
        console.log("Will Number: ",willNo);
          ...
          let someResult = await callContract.methods.modifyPayeeShares(willNumber1,payeeAddress,modifiedShare).send({from:address, gas: 1000000})  
        ...
        <div>
                ...
                Modify Will
				 ...
				 <div>
                    ...
                    <div>Updated Shares(%)</div>
                    <input className="inp_f"
                        type="text"
                        style={{height:"3.0vw", width:"32vw"}}
                        value={modifiedShare}
                        onChange={e => setModifiedShare(e.target.value)}
